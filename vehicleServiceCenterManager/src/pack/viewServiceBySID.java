...
public class viewServiceBySID extends JDialog implements ActionListener {
	...
	public viewServiceBySID(String sID) {
		...
		if(alreadyExists){
			...
			String st="select * from services where ServiceId=?";
			try{
				PreparedStatement ps=co.prepareStatement(st);
				ps.setString(1,sID);
				ResultSet rs=ps.executeQuery();
				while(rs.next()){
					//String sID=rs.getString("ServiceId");//Since We already know Service ID
